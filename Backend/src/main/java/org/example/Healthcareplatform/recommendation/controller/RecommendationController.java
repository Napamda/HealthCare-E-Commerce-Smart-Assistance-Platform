...
@Slf4j
@RestController("productRecommendationController")
...
public class RecommendationController {
    ...
    @GetMapping
    public ResponseEntity<List<ProductResponse>> getRecommendations(Authentication auth) {
        Long userId = Long.parseLong(auth.getName());
        log.info("GET /api/products/recommendations — userId={}", userId);
        // Return popular products as recommendations
        return ResponseEntity.ok(productService.getPopularProducts(10));
    }

    @GetMapping("/personalized")
    public ResponseEntity<List<ProductResponse>> getPersonalizedRecommendations(Authentication auth) {
        Long userId = Long.parseLong(auth.getName());
        log.info("GET /api/products/recommendations/personalized — userId={}", userId);
    ...
    public ResponseEntity<List<ProductResponse>> getRecommendationsByCategory(
            @PathVariable String category,
            Authentication auth) {
        Long userId = Long.parseLong(auth.getName());
        log.info("GET /api/products/recommendations/category/{} — userId={}", category, userId);
