...
const approvedItems = computed(
  () => prescriptionStore.currentPrescription?.items || [],
)
...
async function handleOrderMedications() {
  cartMessage.value = ''
  const ok = await prescriptionStore.orderMedications(prescriptionId.value)
  if (ok) {
    cartMessage.value = 'success'
...
<template>
  <div class="prescription-detail">
    ...
    <div v-else-if="prescriptionStore.currentPrescription" class="detail-content">
      ...
      <div
        v-if="prescriptionStore.currentPrescription.status === 'APPROVED' && approvedItems.length"
        class="detail-card prescribed-meds-card"
      >
        <h3 class="section-title">Your Medications</h3>
        <p class="meds-intro">
          The pharmacist selected these medications for you. Add them to your cart in one step —
          no need to search for each item yourself.
        </p>

        <div class="prescribed-med-list">
          <div
            v-for="item in approvedItems"
            :key="item.productId"
            class="prescribed-med-row"
          >
            <div class="prescribed-med-info">
              <span class="prescribed-med-name">{{ item.productName }}</span>
              <span v-if="item.dosageInstructions" class="prescribed-med-dosage">
                {{ item.dosageInstructions }}
              </span>
            </div>
            <span class="prescribed-med-qty">&times;{{ item.quantity }}</span>
            <span class="prescribed-med-price">{{ formatPrice(item.unitPrice) }}</span>
          </div>
        </div>

        <p v-if="cartMessage === 'success'" class="meds-order-success">
          &#10003; Medications added to your cart.
        </p>
        <p v-else-if="cartMessage === 'error'" class="meds-order-error">
          {{ prescriptionStore.error || 'Could not add medications to cart.' }}
        </p>

        <div class="meds-actions">
          <button
            class="btn btn-primary btn-order-meds"
            :disabled="prescriptionStore.ordering"
            @click="handleOrderMedications"
          >
            <span v-if="prescriptionStore.ordering">Adding...</span>
            <span v-else-if="cartMessage === 'success'">&#10003; Added to Cart</span>
            <span v-else>Add All to Cart</span>
          </button>
          <router-link v-if="cartMessage === 'success'" to="/cart" class="btn btn-view-cart-link">
            View Cart &rarr;
          </router-link>
        </div>
      </div>
