...
function continueChat(consultation) {
  if (consultation.conversationId) {
    router.push(`/chat/${consultation.conversationId}`)
  }
}

function canContinueChat(consultation) {
  return consultation.status === 'ACCEPTED' || consultation.status === 'IN_PROGRESS'
}
</script>
    ...
    <div v-else class="consultations-list">
      ...
      >
        ...
        <div class="card-footer">
          ...
          <button
            v-if="canContinueChat(consultation)"
            class="btn-continue-chat"
            @click="continueChat(consultation)"
          >
            Continue Chat
          </button>
        </div>
