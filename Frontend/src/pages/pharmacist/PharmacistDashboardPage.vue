...
import PrescriptionMedicationPicker from '../../components/prescriptions/PrescriptionMedicationPicker.vue'
...
function viewPrescriptionDetail(prescription) {
  pharmacistStore.fetchPrescription(prescription.id).then(() => {
    ...
    // Pre-seed the picker with any already-selected meds.
    selectedMedications.value = (pharmacistStore.currentPrescription?.items || []).map((i) => ({
...
function downloadFile(prescription) {
  const url = getPrescriptionDownloadUrl(prescription.id)
...
onMounted(() => {
  pharmacistStore.fetchAllPrescriptions()
})
</script>
      ...
      <div class="detail-body">
        ...
        >
          ...
          <div v-if="!pharmacistStore.reviewSuccess">
            ...
            <PrescriptionMedicationPicker v-model="selectedMedications" />
        ...
        <div
          v-if="pharmacistStore.currentPrescription.items && pharmacistStore.currentPrescription.items.length"
          class="detail-card meds-card"
        >
          <h3 class="section-title">Selected Medications</h3>
          <p class="meds-note">These were sent to the patient for ordering.</p>
          <div
            v-for="item in pharmacistStore.currentPrescription.items"
            :key="item.productId"
            class="meds-readonly-row"
          >
            <div class="meds-readonly-info">
              <span class="meds-readonly-name">{{ item.productName }}</span>
              <span v-if="item.dosageInstructions" class="meds-readonly-dosage">{{ item.dosageInstructions }}</span>
            </div>
            <span class="meds-readonly-qty">&times;{{ item.quantity }}</span>
          </div>
        </div>
      </div>
    </div>
